...
int main(void)
{
	...
	xil_printf("--------Testcase 3--------\r\n");
	Status = testcase_3();
	if (Status != XST_SUCCESS) {
		xil_printf("Testcase 3 Failed\r\n");
		return XST_FAILURE;
	}
	xil_printf("--------Testcase 4--------\r\n");
	Status = testcase_4();
	if (Status != XST_SUCCESS) {
		xil_printf("Testcase 4 Failed\r\n");
		return XST_FAILURE;
	}
}
...
int testcase_3() {
	int Status;
	char key[]           = {0xee, 0x84, 0xe1, 0x9c, 0xda, 0x87, 0xa7, 0x62, 0x91, 0xea, 0xaf, 0x20, 0x54, 0xae, 0xf8, 0x12};
	char crypto_header[] = {0x13, 0x36, 0x00, 0x15, 0xf2, 0xcb, 0x94, 0x9b, 0x8f, 0xb0, 0x01, 0x3e, 0x00, 0x00, 0x00, 0x00};
	char plaintext[]     = {0x4d, 0xf6, 0x4b, 0xff, 0x1f, 0xa1, 0x18, 0x95, 0xaf, 0x33, 0x7e, 0xb6, 0x6b, 0x66, 0xe1, 0x29,
							0x1f, 0xda, 0x3c, 0xf8, 0x88};
	char ciphertext[]    = {0xf5, 0x65, 0xa6, 0x8f, 0x46, 0xfc, 0x5e, 0x88, 0xd8, 0xe8, 0xea, 0xcd, 0x2b, 0x39, 0x20, 0xba,
							0x91, 0xa0, 0x33, 0x24, 0xa9};
	...
	for (int i = 0; i < 16; i++) {
		txBuffer[32 + i] = ciphertext[15 - i];
	}
	for (int i = 0; i < 5; i++) {
		txBuffer[48 + i] = ciphertext[20 - i];
	}
	...
	for (int i = 0; i < 16; i++) {
		if (rxBuffer[15 - i] != plaintext[i]) {
			xil_printf("Plaintext incorrect at byte %d\r\n", i);
			return XST_FAILURE;
		}
	}
	for (int i = 0; i < 5; i++) {
		if (rxBuffer[31 - i] != plaintext[16 + i]) {
			xil_printf("Plaintext incorrect at byte %d\r\n", 16 + i);
			return XST_FAILURE;
		}
	}
	...
	xil_printf("Decryption successful\r\n");
...
int testcase_4() {
	int Status;
	char key[]           = {0x47, 0xc4, 0x89, 0x0c, 0xb4, 0x03, 0x44, 0x55, 0x47, 0xb6, 0x4f, 0x6f, 0x8c, 0x85, 0xdc, 0x64};
	char crypto_header[] = {0x12, 0x69, 0x00, 0x44, 0x5b, 0xff, 0x83, 0xf6, 0x76, 0x6d, 0x5e, 0x73, 0x00, 0x00, 0x00, 0x00};
	char plaintext[]     = {0xdb, 0xdd, 0xbe, 0xc4, 0xba, 0x17, 0x19, 0x05, 0xf1, 0x91, 0xf2, 0x73, 0x83, 0xc6, 0xc4, 0x10,
	                        0x11, 0xb8, 0xb1, 0x8b, 0x3f, 0x93, 0xf4, 0xdc, 0xe9, 0xbd, 0x84, 0xd4, 0x2f, 0xc4, 0x36, 0xc2,
	                        0x23, 0x8b, 0x59, 0x74, 0xef, 0x7d, 0xdb, 0xf8, 0xcf, 0xd0, 0x28, 0x4b, 0x89, 0x6f, 0x41, 0x35,
	                        0xc8, 0x33, 0x00, 0x9b, 0xa0, 0x4b, 0x6b, 0xab, 0x28, 0x4e, 0xba, 0xbc, 0x5a, 0x80, 0x47, 0xa6,
	...
	for (int i = 0; i < 16; i++) {
		txBuffer[48 + i] = ciphertext[31 - i];
	}
	for (int i = 0; i < 16; i++) {
		txBuffer[64 + i] = ciphertext[47 - i];
	}
	for (int i = 0; i < 16; i++) {
		txBuffer[80 + i] = ciphertext[63 - i];
	}
	for (int i = 0; i < 4; i++) {
		txBuffer[96 + i] = ciphertext[67 - i];
	}
	...
	for (int i = 0; i < 16; i++) {
		if (rxBuffer[47 - i] != plaintext[32 + i]) {
			xil_printf("Plaintext incorrect at byte %d\r\n", 32 + i);
			return XST_FAILURE;
		}
	}
	for (int i = 0; i < 16; i++) {
		if (rxBuffer[63 - i] != plaintext[48 + i]) {
			xil_printf("Plaintext incorrect at byte %d\r\n", 48 + i);
			return XST_FAILURE;
		}
	}
	for (int i = 0; i < 4; i++) {
		if (rxBuffer[79 - i] != plaintext[64 + i]) {
			xil_printf("Plaintext incorrect at byte %d\r\n", 64 + i);
			return XST_FAILURE;
		}
	}
